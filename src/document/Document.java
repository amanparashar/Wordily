...
public void actionPerformed(ActionEvent e)       //it sees which menu item is selected
{
    ...
    if (retrival == JFileChooser.APPROVE_OPTION) {
		...
		try {
			file = new FileWriter(chooser.getSelectedFile() +".txt"  );//saved the file
       ...
       StyledDocument doc = (StyledDocument)ta.getDocument();           // Take style document
              ...
              //  ElementIterator iterator = new ElementIterator(doc);
                ...
                while (ele>0) {
                    ...
                    AttributeSet ass = element.getAttributes();                    // Takes attribute of each element
                    //System.out.println(ass);
                     // FileWriter fstream = new FileWriter("C:/output.txt");
                       f=0;
                    if (StyleConstants.isBold(ass)) f+=1;
                     if (StyleConstants.isItalic(ass)) f+=3;
                      if (StyleConstants.isUnderline(ass)) f+=5;
                   ...
                   if(!n.equalsIgnoreCase("\n"))
                    ...
                    element= doc.getCharacterElement(++i); ele--;
                }


    	/* try {
			file.write(as.toString(),0, as.length());
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}  */
        	       ...
        	       try
                        {
                            ...
                            char c[];
                            c=line.toCharArray(); //System.out.println(c);
